...
?>
<?php
	$realtimeCommentDivIds = array();
	$totalNodes = array();
	$totalCommentNodes = array();
	
	//echo '<pre>';
	//print_r($realtimeChatCommentDivIds);
	if(count($arrDiscussions) > 0)
	{ ?>

<div style="width:100%;" class="SingleDiscussPage">
  <?php
		...
		foreach($arrDiscussions as $keyVal=>$arrVal)
		{
			...
			$totalCommentNodes[] = $arrVal['nodeId'];	
...
$realtimeCommentDivIds=explode(",",$realtimeChatCommentDivIds);
//print_r($realtimeCommentDivIds);
if(!in_array($arrVal['nodeId'],$realtimeCommentDivIds))	
{
	?><div class="realTimeTopic"><?php
		
			if ($arrVal['chatSession']==0)
			{
		?>
  <div id="chat_block<?php echo $arrVal['nodeId'];?>" class="chat_box" style="padding-left:<?php if ($arrVal['predecessor']>0) {echo 80;}?>px; margin:10px;>
    <div id="<?php echo $position++;?>" style="" class="<?php echo $chatBgColor;?> handCursor" <?php if($arrVal['predecessor'] == 0) { if($timmer) { ?> onclick= "showReply(0,<?php echo $arrVal['nodeId'];?>),nodeId=<?php echo $arrVal['nodeId'];?>;parent.replay_target=0;" <?php }} else {if($timmer) { ?> onclick="showReply(0,<?php echo $arrVal['predecessor'];?>),nodeId=<?php echo $arrVal['predecessor'];?>; parent.replay_target=0;" <?php  }} ?> >
  ...
  <!--Changed by Dashrath- remove <?php echo $chatBgColor;?> class-->
  <div style="width:99%; padding-left:10px;" class="handCursor">
   <div class="clr"></div>
...
<div id="comment_section">
	...
	$arrparent=  $this->chat_db_manager->getPerentInfo($arrVal['nodeId']);
			...
			if($arrparent['successors'])
			{
				...
				$totalCommentNodes[] = $arrDiscussions['nodeId'];				
				//Manoj: getting comment user tag name
				$userDetails2	= 	$this->chat_db_manager->getUserDetailsByUserId($arrDiscussions['userId']);		

$realtimeCommentDivIds=explode(",",$realtimeChatCommentDivIds);
//print_r($realtimeCommentDivIds);
if(!in_array($arrDiscussions['nodeId'],$realtimeCommentDivIds))	
{
				?>
				
				
				<div class="commentSec"><div><?php echo $arrDiscussions['contents']; ?></div>
	...
	<div style="float:right; margin-right:20px;" ><font color="#006699" size="1"><i><?php echo $userDetails2['userTagName'] .'  '.$this->time_manager->getUserTimeFromGMTTime($arrDiscussions['DiscussionCreatedDate'], 'm-d-Y h:i A') ;?></i></font></div></div>
    <div class="clr"></div>
...
<input type="hidden" id="totalNode" value="<?php echo implode(',',$totalNodes);?>">
<input type="hidden" id="totalCommentNodes" value="<?php echo implode(',',$totalCommentNodes);?>">
