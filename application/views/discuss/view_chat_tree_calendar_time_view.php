...
<div style="width:100%;">
  <?php
	...
	{		 
		foreach($arrDiscussions as $keyVal=>$arrVal)
		{
			...
			{
		?>
		<!--Manoj: padding left default 50 in if and 30 in else-->
  <div style="padding-left:<?php if ($arrVal['predecessor']>0) {echo 0;} else {echo 0;}?>px; margin-top:1.5%;">
    ...
    <div style="width:<?php echo $this->config->item('page_width')-130;?>px; padding-left:0px; margin:0.5% 0%; " class="<?php echo $chatBgColor;?> handCursor"> <font color="#006699" size="1"><i><?php echo $userDetails1['userTagName'] .'  '.$this->time_manager->getUserTimeFromGMTTime($arrVal['DiscussionCreatedDate'], 'm-d-Y h:i A') ;?></i></font> </div>
  </div>
  ...
  <?php 				
			...
			{
			?>
  <div style="width:99%; padding-left:10px;" class="<?php echo $chatBgColor;?> handCursor"> <?php echo '<i><font size="1">'.$this->lang->line('txt_Topic').': '.$this->identity_db_manager->formatContent(strip_tags($this->chat_db_manager->getTopicByNodeId($treeId, $arrVal['predecessor'])),130,1).'</font></i>'; ?> </div>
  <?php
			}
			?>
  ...
  <?php
			...
			?>
  <div style="width:99%; margin-left:10px;" class="<?php echo $chatSessionBGColor; ?>">
    <?php 
									if ($arrVal['chatSession']==2)
									{
