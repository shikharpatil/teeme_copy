	...
	if(count($arrDiscussions) > 0)
	{
		...
		foreach($arrDiscussions as $keyVal=>$arrVal)
		{
			...
			if(!empty($attachedTags) && (in_array($arrVal['userId'],$_SESSION['usrLeaves']) || !$_SESSION['usrLeaves'])){
				...
				$i++;?>
<div class="<?php echo $nodeBgColor;?> views_div treeLeafView" style="margin:8px 3px;">
  <?php
  							/*$this->identity_db_manager->formatContent( $arrVal['contents'] ,1000,1)*/
							echo '<div><a href='.base_url().'view_chat/chat_view/'.$treeId.'/'.$workSpaceId.'/type/'.$workSpaceType.'/1/'.$arrVal["nodeId"].'#discussLeafContent'.$arrVal["nodeId"].' style="text-decoration:none; color:#000;">'.stripslashes(substr($arrVal['contents'],0,250)).'</a></div>';
						?>
...
<div class="<?php echo $nodeBgColor;?> views_div treeLeafView" style="margin:8px 3px;">
  <?php
  								/*$this->identity_db_manager->formatContent($arrDiscussions['contents'],1000,1)*/
								echo '<div><a href='.base_url().'view_chat/chat_view/'.$treeId.'/'.$workSpaceId.'/type/'.$workSpaceType.'/1/'.$arrDiscussions["nodeId"].'#discussLeafContent'.$arrDiscussions["nodeId"].' style="text-decoration:none;margin-left:3%; color:#000;">'.stripslashes(substr($arrDiscussions['contents'],0,250)).'</a></div>';
								?>
