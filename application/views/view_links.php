...
<div id="container">
		...
		<div id="content">
            ...
            <tr>
            	...
            	<td align="left" valign="top"><?php echo "<h2>" .$this->lang->line('msg_link_applied_to') .":</h2>";?></td>
...

					{
						...
						//	print_r($arrNodeDetails);
								...
								{ 
									...
									$treeAllContents = '<a href='.base_url().'view_document/index/'.$workSpaceId.'/type/'.$workSpaceType.'/?treeId='.$arrNodeDetails["treeId"].'&doc=exist&node='.$artifactId.'&curVersion='.$curVersion.'&option=1#docLeafContent'.$artifactId.' target="_blank">'.$this->identity_db_manager->formatContent($arrNodeDetails['contents'],250).'</a><hr>';                             
...

								{
									...
									$treeAllContents = '<a href='.base_url().'view_chat/chat_view/'.$arrNodeDetails["treeId"].'/'.$workSpaceId.'/type/'.$workSpaceType.'/1/'.$artifactId.'#discussLeafContent'.$artifactId.' target="_blank">'.$this->identity_db_manager->formatContent($arrNodeDetails['contents'],250).'</a><hr>';
...

								{
									...
									$treeAllContents = '<a href='.base_url().'view_task/node/'.$arrNodeDetails["treeId"].'/'.$workSpaceId.'/type/'.$workSpaceType.'/1/'.$artifactId.'#taskLeafContent'.$artifactId.' target="_blank">'.$this->identity_db_manager->formatContent($arrNodeDetails['contents'],250).'</a><hr>';
...

								{
									...
									$treeAllContents = '<a href='.base_url().'contact/contactDetails/'.$arrNodeDetails["treeId"].'/'.$workSpaceId.'/type/'.$workSpaceType.'/1/'.$artifactId.'#contactLeafContent'.$artifactId.' target="_blank">'.$this->identity_db_manager->formatContent($arrNodeDetails['contents'],250).'</a><br/><br/><hr>';
