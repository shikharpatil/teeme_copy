	...
	for(var i = 0; i < divsToHide.length; i++)
    {
		if (divsToHide[i].style.display=='block')
		{
    		divsToHide[i].style.display='none';
				...
				if (className=='dashboard_content_more_following')
				{
					document.getElementById('more_following').innerHTML = '<?php echo $this->lang->line('see_all_txt'); ?>';
    			}						
		}
		else
		{
			divsToHide[i].style.display='block';
				...
				if (className=='dashboard_content_more_following')
				{
					document.getElementById('more_following').innerHTML = '<?php echo $this->lang->line('see_less_txt'); ?>';
    			}									
		}
...
<div class="dashboard_row dashboard_tree" style="width:33.3%; float:left;" >
	<div class="dashboard_col" >
		...
		<img src=<?php echo base_url(); ?>images/icon_contact_sel15.png alt='<?php echo $this->lang->line('txt_Contact'); ?>' title='<?php echo $this->lang->line('txt_Contact');?>' border=0 <?php if(!(in_array('5',$spaceTreeDetails)) && $treeTypeEnabled==1) { ?> style="display:none;" <?php }  ?>> 
		<?php echo $this->lang->line('txt_Trees');?> </span> 
		<?php 
		if ($count > 5)
		{
		?>
				...
				<div class=<?php echo $class;?> style="margin:10px 0 10px; display:<?php echo $display;?>">
					<div>
						...
						<?php if(in_array($arrVal['type'],$spaceTreeDetails) || $workSpaceId==0 || $workSpaceDetails['workSpaceName']=='Try Teeme') { ?>
						  ...
						  <?php echo $icon; ?> 
						  <!--/*End of Added by Surbhi IV*/	--> 
						  <a href="<?php echo base_url().''.$href; ?>" title="<?php echo $this->identity_db_manager->formatContent($arrVal['name'],0,1);?>"><?php echo $this->identity_db_manager->formatContent($arrVal['name'],150,1); 			 
														if (!empty($arrVal['old_name']))
														{
															echo '<br>(<i>'.$this->lang->line("txt_Previous_Title").':</i> ' .$this->identity_db_manager->formatContent($arrVal['old_name'],150,1).')';
														}?> </a> 
...
<?php if(count($arrPostsTimeline) > 0){ ?>	
	<div class="dashboard_row dashboard_post" style="width:33.3%; float:left;">
		...
		<div class="dashboard_col" >
				...
				foreach($arrPostsTimeline as $keyVal=>$arrVal)
				{
					...
					?>
						...
						<div class="<?php echo $class;?>" style="margin:10px 0 15px;display:<?php echo $display;?>">
							<div>
								<?php 
								$leafdataContent=strip_tags($arrVal['contents']);
								if (strlen($leafdataContent) > 150) 
								{
									$PostTitle = substr($leafdataContent, 0, 150) . "..."; 
								}
								else
								{
...
<?php if(count($arrTalks) > 0){ ?>		
	<div class="dashboard_row dashboard_talk" style="width:33.3%; float:left;">
	<div class="dashboard_col" >
		<!-- Updated talks start -->
		<div class="dashboard_wrap">
		 ...
		 ?>
				...
				<div class="<?php echo $class;?>" style="margin:10px 0 10px;display:<?php echo $display;?>">
					...
					<?php 
										...
										/* End - Get last talk comment */				
								//echo 	'<a href='.base_url().'view_talk_tree/node/'.$data["id"].'/'.$workSpaceId.'/type/'.$workSpaceType.'/ptid/'.$data["parentTreeId"].' target="_blank" class="example7" ><img src='.base_url().'images/tab-icon/talk-view-sel.png alt='.$this->lang->line("txt_Talk").' title="'.$latestTalk.'" border=0></a>&nbsp;'	;
								$leafdataContent=strip_tags($this->identity_db_manager->formatContent($data['name'],50,1));
								...
								$leafhtmlContent=htmlentities($this->identity_db_manager->formatContent($data['name'],50,1), ENT_QUOTES);
								
								?>
								<?php /*onClick="talkOpen('<?php echo $data['id']; ?>','<?php echo $workSpaceId ?>','<?php echo $workSpaceType ?>','<?php echo $data["parentTreeId"] ?>','<?php echo $talkTitle; ?>','0','')"*/ ?>
								
								<input type="hidden" value="<?php echo $leafhtmlContent; ?>" name="talk_content" id="talk_content<?php echo $data['id']; ?>"/>
								<?php			
...
<?php } ?>
	...
	<div class="dashboard_row dashboard_my_tag" style="width:33.3%; float:left; display:none;">
		<div class="dashboard_col" >
	...
	<div class="dashboard_row dashboard_my_task" style="width:33.3%; float:left; display:none; <?php if(!(in_array('4',$spaceTreeDetails)) && $treeTypeEnabled==1) { ?> display:none; <?php }  ?> ">
	<div class="dashboard_col" >
			<!-- My tasks start -->
			<div class="dashboard_wrap">
...
<?php if(count($arrTasks) > 0){ ?>		
	<div class="dashboard_row dashboard_task" style="width:33.3%; float:left; <?php if(!(in_array('4',$spaceTreeDetails)) && $treeTypeEnabled==1) { ?> display:none; <?php }  ?>">
	<!-- All tasks start -->
	<div class="dashboard_col" >
			<div class="dashboard_wrap">
				<?php
									...
									foreach($arrTasks as $key => $arrTagData)
									{
										...
										if (!$checksucc)
										{
											...
											<div class=<?php echo $class;?> style="margin:10px 0 10px; display:<?php echo $display;?>">
												...
												if (strlen($taskdataContent) > 150) 
												{
													$taskTitle = substr($taskdataContent, 0, 150) . "..."; 
												}
												else
												{
...
if($arrAllTagsCount > 0){ ?>
	<div class="dashboard_row dashboard_tag_disabled" style="width:33.3%; float:left; display:none; <?php /*if(!(in_array('4',$spaceTreeDetails)) && $treeTypeEnabled==1) { ?> clear:none; margin-left:3%; <?php } else {  ?> clear:both; <?php } */ ?>">
		<div class="dashboard_col" >
			<!-- Updated tags start -->
			<div class="dashboard_wrap">
...
<?php if(count($arrLinks) > 0){ ?>	
	<div class="dashboard_row dashboard_link_disabled" style="width:33.3%; float:left; display:none;">
		<div class="dashboard_col" >
			<!-- Updated links start -->
			<div class="dashboard_wrap">
...
<?php if(count($externalDocs) > 0){ ?>		
	<div class="dashboard_row dashboard_file" style="width:33.3%; float:left; <?php /*if(!(in_array('4',$spaceTreeDetails)) && $treeTypeEnabled==1) { ?> clear:both; margin-left:0%; <?php } else {  ?> margin-left:3%; <?php }*/  ?> ">
		<div class="dashboard_col" >
...
<div class="dashboard_row dashboard_user" style="width:33.3%; float:left; <?php /*if(!(in_array('4',$spaceTreeDetails)) && $treeTypeEnabled==1) { ?> clear:none; margin-left:3%; <?php } else {  ?> clear:both; <?php }*/  ?>">
		<!-- Recent users start -->
		<div class="dashboard_col" >
				...
				?>
					<div class="<?php echo $class;?>" style="margin:10px 0 10px;display:<?php echo $display;?>">
						<span>
							...
							<span class='clsLabel'><?php if($workSpaceType==2){ /*echo $workPlaceMemberData['tagName'];*/ } else { /*echo $workPlaceMemberData['userTagName'];*/ } ?><?php echo "&nbsp;";?>(Last Login: <?php if($workPlaceMemberData['lastLoginTime']!=''){ echo $this->time_manager->getUserTimeFromGMTTime($workPlaceMemberData['lastLoginTime'],$this->config->item('date_format'),1); } else { echo 'NA'; } ?>)</span>
						</span>
					</div>
				<?php
					$i++;
...
<?php if(count($arrFollowingTrees) > 0){ ?>	
<div class="dashboard_row dashboard_following" style="width:33.3%; float:left;">
	<div class="dashboard_col" >
		<!-- Updated trees start -->
		<div class="dashboard_wrap">
		
		<span class="dashboard_title"><?php /*?><img src=<?php echo base_url(); ?>images/icon_document_sel15.png alt='<?php echo $this->lang->line('txt_Document'); ?>' title='<?php echo $this->lang->line('txt_Document');?>' border=0> <img src=<?php echo base_url(); ?>images/icon_discuss_sel15.png alt='<?php echo $this->lang->line('txt_Discuss'); ?>' title='<?php echo $this->lang->line('txt_Discuss');?>' border=0> <img src=<?php echo base_url(); ?>images/icon_task_sel15.png alt='<?php echo $this->lang->line('txt_Task'); ?>' title='<?php echo $this->lang->line('txt_Task');?>' border=0> <img src=<?php echo base_url(); ?>images/icon_notes_sel15.png alt='<?php echo $this->lang->line('txt_Notes'); ?>' title='<?php echo $this->lang->line('txt_Notes');?>' border=0> <img src=<?php echo base_url(); ?>images/icon_contact_sel15.png alt='<?php echo $this->lang->line('txt_Contact'); ?>' title='<?php echo $this->lang->line('txt_Contact');?>' border=0><?php */?> <?php echo $this->lang->line('txt_trees_following_list');?> </span> 
		<?php 
		$count=count($arrFollowingTrees);
		if ($count > 5)
		{
		?>
			<span><a id="more_following" class="blue-link-underline2" href="javascript:void(0);" onclick="javascript:showHideDashBoard('dashboard_content_more_following');">See all.....</a></span>
		<?php
		}
		?>
			<div class="clr"></div>
			<div style="margin-top:10px;" >
				...
				foreach($arrFollowingTrees as $arrVal)
				{
						$href='';
						$txt_tree_type='';
						$icon='';
						$rowColor = ($i % 2) ? $rowColor1 : $rowColor2; 	
								...
								{
									$display='block';
									$class = 'dashboard_content_following';
								} 
								else 
								{

									$display='none';
									$class = 'dashboard_content_more_following';
								}
								?>
				<div class=<?php echo $class;?> style="margin:10px 0 10px; display:<?php echo $display;?>">
					<div>
							...
							if ($arrVal['type']==1)
							{
								$href='view_document/index/'.$workSpaceId.'/type/'.$workSpaceType.'/?treeId='.$arrVal['id'].'&doc=exist';
								$txt_tree_type=$this->lang->line('txt_Document');
								$icon='<img src="'.base_url().'/images/icon_document_sel15.png" style="margin-right:5px;" title="'.$txt_tree_type.'" />';
							}
							if ($arrVal['type']==2)
							{
								$href='view_discussion/node/'.$arrVal['id'].'/'.$workSpaceId.'/type/'.$workSpaceType;
								$txt_tree_type=$this->lang->line('txt_Discussion');
							}
							if ($arrVal['type']==3)
							{
								if($arrVal['status']==1)
								{
									$href='view_chat/chat_view/'.$arrVal['id'].'/'.$workSpaceId.'/type/'.$workSpaceType;
									
								}
								else
								{
									$href='view_chat/node1/'.$arrVal['id'].'/'.$workSpaceId.'/type/'.$workSpaceType;
								}
								$txt_tree_type=$this->lang->line('txt_Chat');
								$icon='<img src="'.base_url().'/images/icon_discuss_sel15.png" style="margin-right:5px;" title="'.$txt_tree_type.'"/>';
							}
							if ($arrVal['type']==4)
							{
								$href='view_task/node/'.$arrVal['id'].'/'.$workSpaceId.'/type/'.$workSpaceType;
								$txt_tree_type=$this->lang->line('txt_Task');
								$icon='<img src="'.base_url().'/images/icon_task_sel15.png" style="margin-right:5px;" title="'.$txt_tree_type.'"/>';
							}
							if ($arrVal['type']==5)
							{
								$href='contact/contactDetails/'.$arrVal['id'].'/'.$workSpaceId.'/type/'.$workSpaceType;
								$txt_tree_type=$this->lang->line('txt_Contact');
								$icon='<img src="'.base_url().'/images/icon_contact_sel15.png" style="margin-right:5px;" title="'.$txt_tree_type.'"/>';
							}
							if ($arrVal['type']==6)
							{
								$href='notes/Details/'.$arrVal['id'].'/'.$workSpaceId.'/type/'.$workSpaceType;
								$txt_tree_type=$this->lang->line('txt_Notes');
								$icon='<img src="'.base_url().'/images/icon_notes_sel15.png" style="margin-right:5px;" title="'.$txt_tree_type.'"/>';
							}
							?>
						  ...
						  <a href="<?php echo base_url().''.$href; ?>" title="<?php echo $this->identity_db_manager->formatContent($arrVal['name'],0,1);?>"><?php echo $this->identity_db_manager->formatContent($arrVal['name'],150,1); 			 
														?> </a> 
						<span class="clsLabel"><?php echo $this->time_manager->getUserTimeFromGMTTime($arrVal['followDate'], 'm-d-Y h:i A');?></span>
						<?php } } ?>
					</div>
				<!--/* Added by Surbhi IV*/-->
...
<?php } ?>
<!--Following tree list end-->
<!--Following posts list showing start -->
<?php /*?><div class="dashboard_row dashboard_post_following" style="width:30%; margin-left:3%; float:left;">
	<div class="dashboard_col" >
		<!-- Updated trees start -->
		<div class="dashboard_wrap">
		
		<span class="dashboard_title"><img src=<?php echo base_url(); ?>images/message_blue_btn16.png alt='<?php echo $this->lang->line('txt_Post'); ?>' title='<?php echo $this->lang->line('txt_Post');?>' border=0> <?php echo $this->lang->line('txt_posts_following_list');?> </span> 
		<?php 
		$count=count($arrFollowingPosts);
		if ($count > 5)
		{
		?>
			<span><a id="more_following" class="blue-link-underline2" href="javascript:void(0);" onclick="javascript:showHideDashBoard('dashboard_content_more_post_following');">See all.....</a></span>
		<?php
		}
		?>
			<div class="clr"></div>
			<div style="margin-top:10px;" >
				...
				foreach($arrFollowingPosts as $arrVal)
				{
						$href='';
						$txt_tree_type='';
						$icon='';
						$rowColor = ($i % 2) ? $rowColor1 : $rowColor2; 	
								...
								else 
								{
									...
									$display='none';
									$class = 'dashboard_content_more_post_following';
								}
								?>
				<div class=<?php echo $class;?> style="margin:10px 0 10px; display:<?php echo $display;?>">
					<div>
							...
							if ($arrVal['type']=='')
							{
								if($arrVal['workSpaceType']==1)
								{
									if($arrVal['workSpaceId']==$workSpaceId && $arrVal['workSpaceType']==$workSpaceType)
									{
										$workSpaceDetails	= $this->identity_db_manager->getWorkSpaceDetailsByWorkSpaceId($workSpaceId);
										if($arrVal['workSpaceId']==$workSpaceId && $arrVal['workSpaceType']==$workSpaceType)
										{
											$txt_tree_type=$workSpaceDetails['workSpaceName'];
										}
										else
										{
											$txt_tree_type=$workSpaceDetails['workSpaceName'];
										}
										$href='post/index/'.$workSpaceId.'/type/'.$workSpaceType.'/'.$workSpaceId.'/1/0/'.$arrVal['id'].'/#form'.$arrVal['id'];
									}
									else if($arrVal['workSpaceId']!=$workSpaceId && $arrVal['workSpaceType']!=0)
									{
										if ($this->identity_db_manager->isWorkSpaceMember($arrVal['workSpaceId'] ,$_SESSION['userId']) || $this->identity_db_manager->checkManager($_SESSION['userId'],$arrVal['workSpaceId'] ,3))
										{
											$workSpaceDetails	= $this->identity_db_manager->getWorkSpaceDetailsByWorkSpaceId($arrVal['workSpaceId']);
											$txt_tree_type=$workSpaceDetails['workSpaceName'];
											$href='post/index/'.$arrVal['workSpaceId'].'/type/'.$arrVal['workSpaceType'].'/'.$arrVal['workSpaceId'].'/1/0/'.$arrVal['id'].'/#form'.$arrVal['id'];
										}
									}	
								}
								else if($arrVal['workSpaceType']==2)
								{
									if ($this->identity_db_manager->isSubWorkSpaceMember($arrVal['workSpaceId'] ,$_SESSION['userId']))
									{
										$workSpaceDetails	= $this->identity_db_manager->getSubWorkSpaceDetailsBySubWorkSpaceId($arrVal['workSpaceId']);
										$txt_tree_type=$workSpaceDetails['subWorkSpaceName'];
										$href='post/index/'.$arrVal['workSpaceId'].'/type/'.$arrVal['workSpaceType'].'/'.$arrVal['workSpaceId'].'/1/0/'.$arrVal['id'].'/#form'.$arrVal['id'];	
									}
								}
								
								if($arrVal['workSpaceId']==0 && $arrVal['workSpaceType']==0)
								{
									$txt_tree_type=$this->lang->line('txt_Public');
									$href='post/index/'.$workSpaceId.'/type/'.$workSpaceType.'/0/1/public/'.$arrVal['id'].'/#form'.$arrVal['id'];
								}
								//$txt_tree_type=$this->lang->line('txt_Post');
								$icon='<img src="'.base_url().'/images/message_blue_btn16.png" style="margin-right:5px;" title="'.$txt_tree_type.'"/>';
							}
...
<!--Following posts list end-->
	<div class="clr"></div>	
</div>
<!--Dashboard full content end-->
</div>
