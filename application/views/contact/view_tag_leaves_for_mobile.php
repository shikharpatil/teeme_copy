		...
		foreach($ContactNotes as $keyVal=>$arrVal)
		{
			...
			if(!empty($attachedTags) && (in_array($arrVal['userId'],$_SESSION['usrLeaves']) || !$_SESSION['usrLeaves'])){
				...
				if($flag==0){
					...
					$i++;?>
<div class="<?php echo $nodeBgColor;?> views_div treeLeafView" style="margin:8% 0%;">
  <?php
  						/*$this->identity_db_manager->formatContent($arrVal['contents'],1000,1)*/
                        echo '<div><a href='.base_url().'contact/contactDetails/'.$treeId.'/'.$workSpaceId.'/type/'.$workSpaceType.'/1/'.$arrVal["nodeId"].'#contactLeafContent'.$arrVal["nodeId"].' style="text-decoration:none; color:#000;">'.stripslashes(substr($arrVal['contents'],0,250)).'</a></div>';
                        ?>
  <div class="userLabel" style="margin-top:4%;"><?php echo  $userDetails1['userTagName']."&nbsp;&nbsp;"; ?> &nbsp; <?php echo $this->time_manager->getUserTimeFromGMTTime($arrVal['orderingDate'], $this->config->item('date_format')); ?> </div>
</div>
<?php
				}
			}
