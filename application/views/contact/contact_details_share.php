...
<div id="container">
  <div id="leftSideBar" class="leftSideBar"><?php $this->load->view('common/left_menu_bar'); ?>
   ...
   <!--  <div class="menu_new" >
      <ul class="tab_menu_new">
        ...
        <?php
				...
				{
				?>
        <li class="share-view_sel"><a class="active " href="<?php echo base_url()?>contact/contactDetails/<?php echo $treeId;?>/<?php echo $workSpaceId;?>/type/<?php echo $workSpaceType;?>/5" title="<?php echo $this->lang->line('txt_Share');?>" ></a></li>
        <?php
				}
				?>
    ...
    <div id="divSeed" class="seedBackgroundColorNew shareSeedDivHeight">
	    	...
	    	<div>
              	<?php echo $Contactdetail['firstname'].' '.$Contactdetail['lastname']; ?>
            </div>
	</div> 
   ...
   <div class="rightSideContentDiv">
	...
	<div>
		<a class="active 1tab shareCloseIcon" href="<?php echo base_url()?>contact/contactDetails/<?php echo $treeId;?>/<?php echo $workSpaceId;?>/type/<?php echo $workSpaceType;?>/1" title="<?php echo $this->lang->line('txt_Close');?>">
...
function showFilteredMembers()
{
	...
	var val = '';
		...
		if (1)
		{
			...
			?>
				if (toMatch=='')
				{
					val +=  '<input type="checkbox" name="users[]" value="<?php echo $_SESSION['userId'];?>" <?php if (in_array($_SESSION['userId'],$sharedMembers)) { echo 'checked="checked"';}?> <?php if ($Contactdetail['userId']!=$_SESSION['userId']) { echo 'disabled="disabled"';}?>/><?php echo $this->lang->line('txt_Me');?><br>';
				}
			<?php
			}
			else
			...
			?>
				if (toMatch=='')
				{
					val +=  '<input type="checkbox" name="users[]" id="checkAll" onclick="checkAllFunction();" value="0" <?php if ($Contactdetail['userId']!=$_SESSION['userId']) { echo 'disabled="disabled"';}?>/><?php echo $this->lang->line('txt_All');?><br>';
				}
			<?php
			}
			...
			{
				val +=  '<input type="checkbox" name="users[]" value="<?php echo $arrData['userId'];?>" <?php if (in_array($arrData['userId'],$sharedMembers)) { echo 'checked="checked"';}?> <?php if (($Contactdetail['userId']!=$_SESSION['userId']) || ($Contactdetail['userId']==$arrData['userId'])) { echo 'disabled="disabled"';}else{echo ' class="clsCheck remove'.$arrData['userId'].'"';}?> data-myval="<?php echo $arrData['tagName'];?>" /><?php echo $arrData['tagName'];?><br>';
			...
			<?php
				...
				//Get checked userlabel
						if ($Contactdetail['userId']==$_SESSION['userId'])
						{
							if($Contactdetail['userId']!=$arrData['userId'])
							{
			...
			if (str.match(pattern))
			{
				...
				val +=  '<input type="checkbox" name="users[]" value="<?php echo $arrData['userId'];?>" <?php if ($Contactdetail['userId']==$arrData['userId']) { echo 'checked="checked"';}?> <?php if (($Contactdetail['userId']!=$_SESSION['userId']) || ($Contactdetail['userId']==$arrData['userId'])) { echo 'disabled="disabled"';}else{echo ' class="clsCheck remove'.$arrData['userId'].'"';}?> data-myval="<?php echo $arrData['tagName'];?>" /><?php echo $arrData['tagName'];?><br>';
    ...
    <form name="frmCal" action="<?php echo base_url()?>contact/share/<?php echo $treeId;?>/<?php echo $workSpaceId;?>/type/<?php echo $workSpaceType;?>" method="post">
      <table width="100%" border="0" cellspacing="3" cellpadding="3">
        ...
        <tr>
          ...
          <td width="85%" align="left" valign="top" class="tdSpace"><div id="showMem" style="height:120px; width:300px; overflow:scroll;">
              <input type="checkbox" name="users[]" id="checkAll" onclick="checkAllFunction();" value="0" <?php if ($Contactdetail['userId']!=$_SESSION['userId']) { echo 'disabled="disabled"';}?>/>
              ...
              <?php	
							...
							foreach($workPlaceMembers as $arrData)
							{
								...
								{						
							?>
              <input type="checkbox" name="users[]" value="<?php echo $arrData['userId'];?>" <?php if (in_array($arrData['userId'],$sharedMembers)) { echo 'checked="checked"';}?> <?php if (($Contactdetail['userId']!=$_SESSION['userId']) || ($Contactdetail['userId']==$arrData['userId'])) { echo 'disabled="disabled"';}else{echo ' class="clsCheck remove'.$arrData['userId'].'"';}?> data-myval="<?php echo $arrData['tagName'];?>" />
              ...
              <input type="checkbox" name="users[]" value="<?php echo $arrData['userId'];?>" <?php if ($Contactdetail['userId']==$arrData['userId']) { echo 'checked="checked"';}?> <?php if (($Contactdetail['userId']!=$_SESSION['userId']) || ($Contactdetail['userId']==$arrData['userId'])) { echo 'disabled="disabled"';}else{echo ' class="clsCheck remove'.$arrData['userId'].'"';}?> data-myval="<?php echo $arrData['tagName'];?>" />
              <!--Dashrath- code end-->
