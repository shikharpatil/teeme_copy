...
<div>
          ...
          <div id="workPlaceDiv" style="padding-left:0%; margin-top:1%;">
			...
			<td style="padding-bottom:20px;">
				<?php
							...
							?>
			<div>
              <input type="file" name="photo" id="photo" class="text_gre1" size="30" onchange="readURL(this);" onclick="clearSessionMsg();" /><span class="errorMsg photoErrorMsg" style="color:red;"><?php echo $_SESSION['errorMsg']; $_SESSION['errorMsg']='';?></span>
			  </div>
    ...
    <div class="errorMsg" style="margin-left:5px; padding-top:0%;width:100%;color:red;"><?php echo $_SESSION['passwordErrorMsg'];unset($_SESSION['passwordErrorMsg']);?></div>
	...
	<div class="successMsg" style="margin-left:0px; margin-bottom:10px;"><?php echo $_SESSION['preferenceSuccessMsg'];unset($_SESSION['preferenceSuccessMsg']);?></div>
		...
		<div class="emailNotification">
			...
			$spaceDetails = $this->identity_db_manager->getSpaceIdAndTypeByDefaultSpace($Profiledetail['defaultSpace']);
			/*Dashrath- code end*/
			?>
    ...
    <?php
		...
		{				
			...
			?>
				<!--Changed by Dashrath- Add condition $spaceDetails['defaultWorkSpaceType']!=2 -->
				<option value="<?php echo $workSpaceData['workSpaceId'];?>" <?php if($Profiledetail['defaultSpace'] == $workSpaceData['workSpaceId'] && $spaceDetails['defaultWorkSpaceType']!=2) echo 'selected';?><?php if (!($this->identity_db_manager->isWorkSpaceMember($workSpaceData['workSpaceId'],$_SESSION['userId'])) && !($this->identity_db_manager->checkManager($_SESSION['userId'],$workSpaceData['workSpaceId'],3))) echo 'disabled';?>><?php echo $workSpaceData['workSpaceName'];?></option>

				<!--Added by Dashrath- for sub workspace add in dropdown-->
				<?php
	...
	?>
		...
		{				
			...
			?>
				<!--Changed by Dashrath- Add condition $spaceDetails['defaultWorkSpaceType']!=2 -->
				<option value="<?php echo $workSpaceData['workSpaceId'];?>" <?php if(($Profiledetail['defaultSpace'] == $workSpaceData['workSpaceId'] && $spaceDetails['defaultWorkSpaceType']!=2) || ($workSpaceData['workSpaceId'] == $workSpaceId && $workSpaceType!=2)) echo 'selected';?>><?php echo $workSpaceData['workSpaceName'];?></option>
				...
				if(count($subWorkspaceDetails) > 0)
				{
					...
					{	
						...
						{
							?>
							<option value="s,<?php echo $workSpaceData['subWorkSpaceId'];?>" <?php if($workSpaceData['subWorkSpaceId'] == $spaceDetails['defaultWorkSpaceId'] && $spaceDetails['defaultWorkSpaceType']==2) echo 'selected';?>><?php echo ' >>> <b>' .$workSpaceData['subWorkSpaceName'] .'</b>';?></option>
							<?php
						}
					}
				}
