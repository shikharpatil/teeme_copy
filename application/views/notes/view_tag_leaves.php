	...
	if(!empty($attachedTags) && (in_array($treeDetail['userId'],$_SESSION['usrLeaves']) || !$_SESSION['usrLeaves'])){
		...
		if($flag==0){?>
            ...
            <?php
                ...
                echo '<a href="'.base_url().'notes/Details/'.$treeId.'/'.$workSpaceId.'/type/'.$workSpaceType.'/1/'.$treeId.'/'.$tag.'/'.$tagId.'/'.$tagType.'" style="text-decoration:none; color:#000;">'.stripslashes(substr($treeDetail['name'],0,1000)).'</a>';
