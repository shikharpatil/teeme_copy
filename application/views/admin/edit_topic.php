          ...
          <tr>
            <td align="left" valign="top" bgcolor="#FFFFFF"><table width="<?php echo $this->config->item('page_width')-55;?>" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<!-- Main Body -->
				    ...
				    <tr>
				      <td class="subHeading" colspan="4"><strong><?php echo $this->lang->line('txt_Help_Topic_Details');?>:</strong></td>
				    </tr>
                    
              
                    <tr>
                      <td class="subHeading" colspan="4">
						<form name="frmTopic" action="<?php echo base_url().'admin/admin_help/edit_topic/'.$arrTopics['topicId'];?>" method="post">
                        ...
                        <tbody>
                          <tr>
                            <td width="179" align="left" valign="middle" class="text_gre1"><?php echo $this->lang->line('txt_Topic_Name');?>:<span class="text_red">*</span></td>
                            <td width="5" align="center" valign="middle" class="text_gre"><strong>:</strong></td>
                            <td width="512" align="left" valign="top" class="text_gre1">
                              <input name="topic" class="text_gre1" id="topic" size="30" value="<?php echo $arrTopics['topicName'];?>"/>                             
                            </td>
                          </tr>
                          <tr valign="top">
                            <td width="179" align="left" class="text_gre1"><?php echo $this->lang->line('txt_Topic_Trees');?><span class="text_red">*</span></td>
                            <td width="5" align="center" class="text_gre"><strong>:</strong></td>
                            <td width="512" align="left" class="text_gre">
                              <select name="trees[]" id="trees" multiple="multiple">
                                <option value="DOC" <?php if(substr_count($arrTopics['trees'],'DOC') > 0) { echo 'selected'; }?>>Document</option>
                                <option value="DIS" <?php if(substr_count($arrTopics['trees'],'DIS') > 0) { echo 'selected'; }?>>Discussion</option>
                                <option value="CHAT" <?php if(substr_count($arrTopics['trees'],'CHAT') > 0) { echo 'selected'; }?>>Chat</option>
                                <option value="ACT" <?php if(substr_count($arrTopics['trees'],'ACT') > 0) { echo 'selected'; }?>>Activity</option>
                                <option value="NOT" <?php if(substr_count($arrTopics['trees'],'NOT') > 0) { echo 'selected'; }?>>Notes</option>
                                <option value="CON" <?php if(substr_count($arrTopics['trees'],'CON') > 0) { echo 'selected'; }?>>Contact</option>
                                <option value="TAG" <?php if(substr_count($arrTopics['trees'],'TAG') > 0) { echo 'selected'; }?>>Tags</option>
								<option value="Oth" <?php if(substr_count($arrTopics['trees'],'Oth') > 0) { echo 'selected'; }?>>Other</option>
                              </select>
                            </td>
                          </tr>
                          <tr>
