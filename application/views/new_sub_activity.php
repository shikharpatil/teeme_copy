...
{	
	...
	{
		window.location = baseUrl+'view_activity/node_activity/'+document.getElementById('nodeId').value+'/'+workSpaceId+'/type/'+workSpaceType;
	}
}
...
function request_refresh(){
	...
	//	alert(url);
	  ...
	  <tr>
		...
		<!--div id="chat_msg" style="width:100%; height:300px; overflow:scroll;">&nbsp;</div-->
		<form name="form1" method="post" action="<?php echo base_url();?>new_activity/start_sub_activity/<?php echo $pnodeId;?>/0/<?php echo $workSpaceId;?>/type/<?php echo $workSpaceType;?>/<?php echo $linkType;?>"><table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr>
    <td colspan="3">
<?php 
$treeStatus = 0;

$treeDetail = $this->activity_db_manager->getDiscussionDetailsByNodeId( $pnodeId );
$treeId = $treeDetail['treeId'];
if($this->uri->segment(8) != '' && $this->uri->segment(8) > 0)
{
	$treeStatus = 1;
	$nodeId		= $this->uri->segment(8);	
	$arrDiscussions	= $this->activity_db_manager->getNodesByPredecessor($nodeId);
	if(count($arrDiscussions) > 0)
	{	
		?>
		<table width="100%">
		<tr><td>
			<span class="style1"><?php echo $treeDetail['contents'];?></span>
			</td></tr>
		<tr><td>
			<hr>
			</td></tr>
...
$cancelUrl = base_url().'view_activity/node_activity/'.$pnodeId.'/'.$workSpaceId.'/type/'.$workSpaceType;
?>
	<input type="button" name="Replybutton" value="<?php echo $this->lang->line('txt_Add_More');?>" onClick="changeOption(2),validate_dis();">
  ...
  <tr>
    <td colspan="3">
		...
		<input name="nodeId" type="hidden" value="<?php echo $pnodeId;?>">				
	</td>
	</tr>
</table>                
</form>
