...
<div id="container">
  	...
  	<div>
                <ul class="tab_menu_new_for_mobile" style="z-index:1000;margin-left:5px;">
                	...
                	<li>
                    	...
                    	<a style="padding:10px 18px;" href="javascript:void(0);" id="tabm1" class="active" onclick="$('#TimelineRightContentForTab').show();$('#onlineUsersForTab').hide();$('#tabm2').removeClass('active');$('#tabm1').addClass('active');"><img src="<?php echo base_url();?>images/message_gray_btn.png" /></a>

                    </li>

                	<li>

                		<a style="padding:10px 18px;" href="javascript:void(0);" id="tabm2" onclick="$('#TimelineRightContentForTab').hide();$('#onlineUsersForTab').show();$('#tabm1').removeClass('active');$('#tabm2').addClass('active');"><img src="<?php  echo base_url().'images/list_icon.png'; ?>" style="margin-bottom:5px;" /></a>

                    </li>		

                </ul>
		</div>
		<?php /*?><div style="float:left;width:100%;height:15px;" >
  ...
  <div style="width:100%;">
		...
		<div id="onlineUsersForTab" style="display:none; border-top:1px solid #ccc;">
		...
		?>
        	...
        	<!--<input type="text" name="search" id="search" value="Search"  onKeyUp="showSearchUser()" onclick="removeSearh()"  onblur="writeSearh()" style="width:98%"/>-->
		<?php
		}
		?>
...
<!--Online users code end here-->
  	...
  	<div id="TimelineRightContentForTab" style="width:100%;">
 	
		
		<!--Space and place section start here-->
		<?php /*?><div style="margin-top:0%;">
		<div class="menu_new" style="background:white;">
		
		<select name="postTypeSelect" id="postTypeSelect" class="selbox-post" style="width:40%;" onChange="javascript:selectPostType(this,'<?php echo $workSpaceId;?>','<?php echo $workSpaceType;?>');">
			
			<?php if ($myProfileDetail['userGroup']>0) { ?>
		
		<option value="space"><a style="margin-top: 9px;padding-left:11px;<?php if(!$_SESSION['all'] && !$_SESSION['public']) { ?> background:#ECECEC;" class="active <?php } ?>" title="<?php echo $workSpaceName ?>"  id="curr" href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType; ?>/<?php echo $workSpaceId; ?>/<?php echo $workSpaceType; ?>"><span>
        ...
        <option value="all" <?php if($this->uri->segment(8)=='all') echo 'selected';?> ><a href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType; ?>/0/1/all" style="margin-top: 9px;padding-left:11px;  <?php if($_SESSION['all']) { ?>background:#ECECEC;" class="active <?php } ?>" id="all"><?php echo $this->lang->line('all_post_txt').' ('.$allPosts.')'; ?></a></option>
		<?php } ?>
		
		<!--For public posts start-->
		<option value="public" <?php if($this->uri->segment(8)=='public') echo 'selected';?>>
			<a href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType; ?>/0/1/public" style="margin-top: 9px;padding-left:11px;  <?php if($_SESSION['public']) { ?>background:#ECECEC;" class="active <?php } ?>" id="public"><?php echo $this->lang->line('public_txt').' ('.$totalPublicPosts.')'; ?></a>
		</option>
		
		<option value="bookmark" <?php if($this->uri->segment(8)=='bookmark') echo 'selected';?>>
			<a style="margin-top: 9px;padding-left:11px;" id="bookmark"><?php echo $this->lang->line('txt_object_following').' ('.$totalBookmarkPosts.')'; ?></a>
  ...
  <div>
    ...
    <ul class="tab_menu_new">
		...
		<a style="padding-left:12px;margin-left:2px;<?php if(!$_SESSION['all'] && !$_SESSION['public'] && $this->uri->segment(8)!='bookmark') { ?> background:#ECECEC;" class="active <?php } ?>" title="<?php echo $workSpaceName ?>"  id="curr" href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType; ?>/<?php echo $workSpaceId; ?>/<?php echo $workSpaceType; ?><?php if($this->uri->segment(10)!=''){ echo '/0/0/'.$userPostSearch; }?>"><span>
		  ...
		  <?php } else if ($workSpaceId>0){ ?>
		<a style="padding-left:12px;margin-left:2px;<?php if(!$_SESSION['all'] && $this->uri->segment(8)!='bookmark') { ?> background:#ECECEC;" class="active <?php } ?>" title="<?php echo $workSpaceName ?>"  id="curr" href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType; ?>/<?php echo $workSpaceId; ?>/<?php echo $workSpaceType; ?>"><span>
...
<?php if($myProfileDetail['userGroup']>0){ ?>
		...
		<li>
			...
			<a href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType; ?>/0/1/bookmark<?php if($this->uri->segment(10)!=''){ echo '/0/'.$userPostSearch; }?>" style="padding-left:12px;margin-left:17px;<?php if($this->uri->segment(8)=='bookmark') { ?>background:#ECECEC;" class="active <?php } ?>" id="bookmark"><?php echo $this->lang->line('txt_post_starred').' ('.$totalBookmarkPosts.')'; ?></a>
		 ...
		 <li>
           ...
           <?php /*?> <a href="javascript:void(0);" id="profile" style="padding-left:12px;margin-left:17px;" title="profile" class="<?php if($profileForm=='1' && $passwordForm!='1'){ ?> active <?php } ?>" onclick="$('#profile').addClass('active');$('#passwordForm').removeClass('active');$('#notification').removeClass('active');$('#password_form').hide();$('#notificationSection').hide();$('#profileForm').show(); clearSessionMsg();"><?php echo $this->lang->line('profile_txt'); ?></a><?php */?>
			...
			<?php if ($workSpaceDetails['workSpaceName']!="Try Teeme") { ?>
            <a href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType; ?>/0/1/all<?php if($this->uri->segment(10)!=''){ echo '/0/'.$userPostSearch; }?>" style="padding-left:12px;margin-left:17px; <?php if($_SESSION['all']) { ?>background:#ECECEC;" class="active <?php } ?>" id="all"><?php echo $this->lang->line('all_post_txt').' ('.$allPosts.')'; ?></a></option>
		<?php } ?>
	...
	<!--Tab section end here-->
		...
		<div id="TimelineProfile" style=" display: <?php if($this->uri->segment(10)==''){ echo 'none';}else{ echo 'block';}?> "> 
			<div style="float:left; width:100%;">
				<div class="timelineProfImg" style="float:left; width:60px !important;">
			...
			<?php } ?>
				...
				<?php /*?><table style="font-size:0.8em; width:80%;">
				<?php if($Profiledetail['firstName']!='' && $Profiledetail['lastName']!='') { ?>
					<tr>
			<td style="padding-bottom:5px;" class="profileLeftLabel">
				<?php echo $this->lang->line('txt_user_full_name');?>:
			...
			<?php } ?>
				...
				<?php /* ?>
				<div class="timelineProfname" style="padding-left:70px; padding-top:2px; font-size:0.8em;">
					<b><?php echo strip_tags($Profiledetail['firstName'].' '.$Profiledetail['lastName'],'<b><em><span><img>'); ?></b>
				</div>
				<?php if($Profiledetail['statusUpdate']!='') { ?>
				<div class="timelineProfname" style="padding-left:70px; padding-top:6px; font-size:0.8em;">
		...
		<div>
		<div class="leftAddBox" style="width:20%;">
		...
		<!--Plus icon for public post end here-->
			...
			<div class="rightAddBox" style="width:80%;">
					...
					<!--Show all post(s) created by user-->
							<?php 
								if($this->uri->segment(10)!='')
								{
										...
										?>
										<a style="float:right;" href="<?php echo base_url(); ?>post/index/<?php echo $workSpaceId; ?>/type/<?php echo $workSpaceType.$showAllUrl; ?>"><?php echo $this->lang->line('txt_show_all_post'); ?></a>
										</span>
										<?php
									}
								?>
		...
		<div id="TimelineEditor" style="width:98%; padding:1%; display:none;" >
			<form name="formTimeline" id="formTimeline" method="post" action="" >
				 ...
				 <?php
					if($workSpaceId=='0' && $_SESSION['public'] != 'public')
					{
						...
						<div id="showManGroup" style="max-height:150px;margin-left:50px; overflow:scroll; margin-bottom:30px; margin-top:20px; width:65%; ">
			...
			<?php } ?>			
						...
						<div id="showMan" style="max-height:120px;margin-left:50px; overflow:scroll; margin-bottom:15px; margin-top:20px; ">
...
function selectPostType(postType,workSpaceId,workSpaceType)
{
	...
	{
		url = baseUrl+'post/index/'+workSpaceId+'/type/'+workSpaceType+'/'+workSpaceId+'/'+workSpaceType;
	}
	else if(postSpaceType=="all")
	{
		url = baseUrl+'post/index/'+workSpaceId+'/type/'+workSpaceType+'/0/1/all';
	}
	else if(postSpaceType=="public")
	{
		url = baseUrl+'post/index/'+workSpaceId+'/type/'+workSpaceType+'/0/1/public';
	}
	else if(postSpaceType=="bookmark")
	{
		url = baseUrl+'post/index/'+workSpaceId+'/type/'+workSpaceType+'/0/1/bookmark';
	}
			...
			{*/
				...
				success:  function(result){
				  ...
				  if(result==1)
				  {
					if($bookmarkStatus=='unbookmark')
					{
						$('.bookmarkBtn'+nodeId).html('<a class="bookmark" onclick="add_bookmark('+nodeId+',\'bookmark\')"><img style="cursor:pointer;height:23px;border:0px;"  src="<?php echo base_url();?>images/bookmark.png"><?php //echo $this->lang->line('txt_object_follow'); ?></a>');
					}
				  }
				}
				});
				...
				$.ajax({
					...
					success:  function(result){
					  ...
					  if(result==1)
					  {
						...
						{
							$('.bookmarkBtn'+nodeId).html('<a class="bookmarked marked'+nodeId+'" onclick="add_bookmark('+nodeId+',\'unbookmark\')" ><img style="cursor:pointer;height:23px;border:0px;"  src="<?php echo base_url();?>images/bookmarked.png"><?php //echo $this->lang->line('txt_object_following'); ?></a>');
						}
					  }
					}
				});
...
function changeBookmarkStatusOut(nodeId)
{
	...
	if(ismobile!=true && istablet!=true)
	{
		...
		$('.marked'+nodeId).text('<?php echo $this->lang->line('txt_object_following') ?>');
	}
}
//check all function start
